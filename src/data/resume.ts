...
파이프라인 설계까지 경험했습니다.`,
  ...
  projects: [
    {
      title: '01. LocalLLM-OCR project (의회문서요약프로젝트)',
      subject: '의회 문서 OCR 기반 요약 및 질의응답 시스템',
      period: '2026.03 ~ 진행중',
      role: '백엔드 1인 개발',
      description: `메인 서버(도메인)와 LLM Worker(요약/RAG)를 분리한 로컬 LLM 아키텍처를 설계·구축하고, 요약 파이프라인·RAG·권한 필터를 Worker 계층에 집약해 운영 안정성과 확장성을 확보`,
      techStack: {
      ...
      responsibilities: [
        'POST /api/vllm/process 중심 장문 요약 파이프라인을 설계·구현(청킹, 부분요약, 부분 재요약, 최종요약, 메타데이터 생성)',
        '문서 길이·GPU 메모리 제약을 반영한 토큰 예산 정책, chunk stride, 단계 분할 기준을 수립하고 운영 문서화',
        'RAG를 qa/summary 이중 모드로 분리해 목적별 컨텍스트 수집 전략을 적용',
        'RBAC 메타 필터(access_level, owner_user_id)와 파일 단위 격리 규칙으로 권한 범위 내 컨텍스트만 사용하도록 설계',
        'vLLM 연동, SSE 응답 흐름, .env 기반 동시성·샘플링 옵션을 운영 환경에서 조정 가능하게 구성',
        'UI/UX 개선(메인 페이지): 메인페이지 영역을 직접 설계·구현하고, 정보 구조 재정의·컬러/컴포넌트 톤 일관화·요약 스트리밍 상태(진행/완료/오류) 가시화·핵심 액션 동선 최적화로 실제 사용 흐름을 개선',
      ],
      achievements: [
        '단계형 요약 파이프라인으로 최대 60페이지 문서 요약 처리',
        '메인 서버와 Worker 분리 구조로 운영 안정성·확장성 확보',
        '요약 API 스트리밍으로 진행 상태/결과를 순차 전달해 사용자 대기 체감 개선',
      ],
    },
    {
