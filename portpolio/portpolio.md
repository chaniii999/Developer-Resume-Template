...
실시간 서비스와 문서 AI 파이프라인을 안정적으로 운영 가능한 구조로 설계하는 백엔드 엔지니어입니다.
...
| 담당 업무 | WebSocket + STOMP + RabbitMQ 기반 실시간 메시징 아키텍처 설계·구현<br>Electron 데스크톱 앱 개발, 앱 전체 컨셉 UI/UX 설계·구현<br>스레드풀·캐싱·브로커 튜닝, MongoDB 인덱스 및 쿼리 최적화 |
| 성과 | RabbitMQ로 1:1·그룹 채팅·메시지 검색 등 메시징 구현 / STOMP 인터셉터로 채팅방 보안·인증<br>p95 지연시간 27% 단축(91.9ms→67ms), 최대 지연시간 72% 개선, 300명 동시 접속 송수신 성공률 100% |
