...
- LocalLLM-OCR project(의회문서요약프로젝트): 메인 서버-Worker 분리 아키텍처로 요약/RAG/권한 필터를 모듈화해 운영 안정성과 확장성 중심으로 설계·구현
...
| 프로젝트 기간 | 2026.03 ~ 진행중 |
| 역할 및 인원 | 백엔드 1인 개발 |
| 프로젝트 소개 | 메인 서버(도메인)와 LLM Worker(요약/RAG)를 분리한 로컬 LLM 아키텍처를 설계·구축하고, 요약 파이프라인·RAG·권한 필터를 Worker 계층에 집약해 운영 안정성과 확장성을 확보했습니다. (3060 12GB 단일 GPU 환경 기준 운영) |
...
| 담당 업무 | `POST /api/vllm/process` 요약 파이프라인 구현(청킹→부분요약→최종요약→메타데이터), `POST /api/chat/question` QA 경로 구현, RAG 이중 모드(qa/summary) 설계, RBAC 메타 필터(`access_level`/`owner_user_id`) 적용, vLLM 연동/동시성/토큰 예산 운영 튜닝 |
